import { ObjectId, Transaction, TransactionData, TransactionKind, GasData, TransactionData_v26, RpcApiVersion } from '../../types';
...
export declare class LocalTxnDataSerializer implements TxnDataSerializer {
    ...
    serializeToBytes(signerAddress: string, txn: UnserializedSignableTransaction, _mode?: TransactionBuilderMode): Promise<Uint8Array>;
    /**
     ...
     */
    serializeToBytesWithoutGasInfo(signerAddress: string, txn: UnserializedSignableTransaction): Promise<Uint8Array>;
    ...
    constructTransactionData(signerAddress: string, unserializedTxn: UnserializedSignableTransaction): Promise<TransactionData | TransactionData_v26>;
    /**
     ...
     */
    serializeTransactionData(tx: TransactionData | TransactionData_v26, size?: number): Promise<Uint8Array>;
    /**
     * Serialize `TransactionKind` into BCS encoded bytes
     */
    serializeTransactionKind(tx: TransactionKind, size?: number): Promise<Uint8Array>;
    /**
     * Deserialize BCS encoded bytes into `SignableTransaction`
     */
    deserializeTransactionBytesToSignableTransaction(bytes: Uint8Array): Promise<UnserializedSignableTransaction | UnserializedSignableTransaction[]>;
    /**
     * Deserialize `TransactionData` to `SignableTransaction`
     */
    transformTransactionDataToSignableTransaction(tx: TransactionData | TransactionData_v26): Promise<UnserializedSignableTransaction | UnserializedSignableTransaction[]>;
    transformTransactionToSignableTransaction(tx: Transaction, gasData: GasData): Promise<UnserializedSignableTransaction>;
}
